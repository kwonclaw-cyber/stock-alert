...
export default function EnhancePage() {
  ...
  const cards = data.enhancements;

  async function setImage(id: string, files: FileList | File[]) {
    const f = Array.from(files).find((x) => x.type.startsWith("image/"));
    if (!f) return;
    const url = await fileToDataUrl(f, 2000);
    update((d) => { const c = d.enhancements.find((x) => x.id === id); if (c) c.image = url; });
  }

  return (
    <div className="mx-auto max-w-5xl">
      <PageHelp>
        <b>강화 정보</b>를 카드로 모아두는 곳이에요(방어구·무기·반지 강화, 강화석 제작 등). <b>카드 생성</b> 후 카드 안을 클릭하고 <b>Ctrl+V</b>로 캡처를 붙여넣으세요(드래그·파일선택도 가능). 이미지는 <b>클릭하면 원본 크기</b>로 볼 수 있어요. 제목·메모도 적어두세요.
      </PageHelp>

      <div className="mb-4 flex items-center justify-between">
        <span className="text-sm text-white/50">총 {cards.length}장</span>
        <Btn variant="primary" onClick={() => update((d) => { d.enhancements.push({ id: uid(), title: "", image: "", memo: "" }); })}>
          + 카드 생성
        </Btn>
      </div>
          ...
          return (
            <div key={card.id} className="overflow-hidden rounded-xl border border-white/10 bg-[#15171c]">
              <div className="flex items-center gap-2 border-b border-white/10 px-3 py-2">
                <TextInput
                  value={card.title}
                  onChange={(v) => update((d) => { d.enhancements[ci].title = v; })}
                  placeholder="제목 (예: 방어구 강화 / 무기 강화 / 반지 강화)"
                  className="!text-left flex-1 font-semibold"
                />
                <button onClick={() => update((d) => { d.enhancements.splice(ci, 1); })} className="text-red-300/60 hover:text-red-300" title="카드 삭제">삭제</button>
              </div>

              <CardImage image={card.image} onFiles={(f) => setImage(card.id, f)} onZoom={() => card.image && setZoom(card.image)} onRemove={() => update((d) => { d.enhancements[ci].image = ""; })} />

              <div className="space-y-2 px-3 py-2">
                <TextArea
                  value={card.memo}
                  onChange={(v) => update((d) => { d.enhancements[ci].memo = v; })}
                  placeholder="메모 (확률, 제련비, 옵션, 강화석 재료 등)"
                  rows={2}
                />
              </div>
            </div>
      ...
      {cards.length === 0 && (
        <p className="mt-6 rounded-lg border border-dashed border-white/15 py-12 text-center text-sm text-white/30">
          카드가 없습니다. “카드 생성”을 눌러 강화표 캡처를 붙여넣어 보세요.
        </p>
      )}
